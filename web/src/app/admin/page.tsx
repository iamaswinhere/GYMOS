          ...
          <div key={i} className={`dashboard-card relative overflow-hidden group border-white/5 bg-[#0D0D0D] p-5 md:p-6`}>
             <div className="relative z-10 flex justify-between items-start">
               <div>
                  ...
                  <h3 className="text-2xl md:text-3xl font-black text-white tracking-tight">{stat.value}</h3>
               </div>
               <div className={`w-10 h-10 md:w-12 md:h-12 bg-white/5 rounded-xl md:rounded-2xl flex items-center justify-center border border-white/5 group-hover:border-primary/50 transition-all ${stat.isTraffic && gymTraffic > 85 ? 'text-red-500 relative' : 'text-primary'}`}>
                  <stat.icon size={20} className={stat.isTraffic && gymTraffic > 85 ? 'animate-pulse' : ''} />
                  {stat.isTraffic && gymTraffic > 85 && (
                    <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-red-500 rounded-full animate-ping"></span>
                  )}
               </div>
             </div>
             <div className={`absolute -right-4 -bottom-4 w-20 h-20 md:w-24 md:h-24 bg-gradient-to-br ${stat.color} blur-3xl opacity-30`}></div>
          </div>
        ))}
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        <div className="lg:col-span-2 dashboard-card bg-[#0D0D0D] p-4 md:p-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
            ...
            <div className="flex bg-black/40 p-1 rounded-xl border border-white/5 w-fit">
                <button onClick={() => setViewType('weekly')} className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase transition-all ${viewType === 'weekly' ? 'bg-primary text-black shadow-lg shadow-primary/20' : 'text-gray-500 hover:text-white'}`}>Weekly</button>
          ...
          <div className="h-[250px] md:h-[350px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                <XAxis dataKey="name" stroke="#444" fontSize={9} tickLine={false} axisLine={false} tick={{ fill: '#444' }} />
                <YAxis hide />
                <Tooltip 
                  contentStyle={{ backgroundColor: '#0D0D0D', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '16px', color: '#fff', fontSize: '12px' }}
                  itemStyle={{ color: '#ffc400', fontWeight: 'bold' }}
                  formatter={(value: any) => [`₹${value.toLocaleString()}`, 'Income']}
                />
                <Area type="monotone" dataKey="income" stroke="#ffc400" strokeWidth={3} fillOpacity={1} fill="url(#colorIncome)" animationDuration={1000} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          {highlightedMonth && viewType === 'monthly' && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-6 p-4 bg-primary/5 border border-primary/20 rounded-2xl flex items-center gap-4">
               <div className="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center shrink-0">
                    <Sparkles size={14} className="text-primary" />
               </div>
               <p className="text-[11px] text-gray-300 font-medium leading-relaxed">Agent Insight: <span className="text-primary font-black uppercase tracking-tighter">Seasonality Alert!</span> {highlightedMonth} shows lower predicted revenue. AI suggests a summer discount campaign.</p>
            </motion.div>
          )}
        </div>

        <div className="space-y-6">
          <div className="dashboard-card border-primary/20 bg-primary/5 relative overflow-hidden group p-6">
            <div className="relative z-10">
                <h3 className="text-xs font-black text-primary uppercase tracking-widest mb-4 flex items-center gap-2">
                <Sparkles size={14} strokeWidth={3} /> AI Performance Reporter
                </h3>
                {aiReport ? (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                    <p className="text-[11px] text-gray-400 leading-relaxed font-bold border-l-2 border-primary/30 pl-3 py-1 bg-primary/5">{aiReport.summary}</p>
                    <div className="space-y-2.5">
                        {aiReport.insights.map((insight, idx) => (
                            <div key={idx} className="flex items-start gap-2">
                                <ArrowUpRight size={14} className="text-primary/70 shrink-0 mt-0.5" />
                                <p className="text-[10px] text-white/80 font-medium">{insight}</p>
                            </div>
                        ))}
                    </div>
                    <div className="mt-4 pt-4 border-t border-primary/10">
                        <p className="text-[9px] font-black text-primary uppercase tracking-widest mb-1.5 opacity-60">Growth Opportunity</p>
                        <p className="text-[10px] text-white italic leading-relaxed">"{aiReport.growthOpportunity}"</p>
                    </div>
                    <button onClick={() => generateAIReport()} className="text-[9px] font-black text-primary/50 hover:text-primary transition-colors uppercase tracking-widest mt-4">Refresh Analysis</button>
                </motion.div>
                ) : (
                <div className="py-8 text-center bg-black/40 rounded-2xl border border-white/5">
                    ...
                    <button onClick={() => generateAIReport()} className="bg-primary text-black px-5 py-2.5 rounded-xl text-[10px] font-black uppercase shadow-[0_0_20px_rgba(255,196,0,0.15)] hover:bg-white transition-all">Generate AI Report</button>
                </div>
                )}
            </div>
          </div>
          <div className="dashboard-card bg-[#0D0D0D] p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xs font-black text-white uppercase tracking-widest">Recent Activity</h3>
              <button onClick={() => router.push('/admin/members')} className="text-[10px] text-primary/50 hover:text-primary font-black uppercase tracking-tighter transition-colors">Directory</button>
            </div>
            <div className="space-y-5">
              {payments.length === 0 ? (
                <div className="py-12 flex flex-col items-center gap-3">
                   <Activity className="text-gray-900" size={32} />
